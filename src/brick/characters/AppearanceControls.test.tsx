...
describe('figure appearance controls', () => {
  ...
  it('focuses sections without losing controlled choices or exposing unrelated options', () => {
    const props = { locked: new Set<string>(), onToggleLock: vi.fn(), onChange: vi.fn() }
    const { rerender } = render(<AppearanceControls {...props} section="head" />)
    expect(screen.getByRole('group', { name: 'Face' })).toBeInTheDocument()
    expect(screen.getByRole('group', { name: 'Skin tone' })).toBeInTheDocument()
    expect(screen.queryByRole('group', { name: 'Outfit' })).not.toBeInTheDocument()
    rerender(<AppearanceControls {...props} section="outfit" />)
    expect(screen.getByRole('group', { name: 'Body' })).toBeInTheDocument()
    expect(screen.getByRole('group', { name: 'Outfit' })).toBeInTheDocument()
    expect(screen.queryByRole('group', { name: 'Skin tone' })).not.toBeInTheDocument()
    fireEvent.click(screen.getByRole('button', { name: 'Overalls' }))
    expect(props.onChange).toHaveBeenLastCalledWith({ ...DEFAULT_CHARACTER_APPEARANCE, outfit: 'overalls' })
    rerender(<AppearanceControls {...props} section="extras" />)
    expect(screen.getByRole('button', { name: 'Backpack' })).toBeInTheDocument()
    expect(screen.queryByRole('group', { name: 'Body' })).not.toBeInTheDocument()
    rerender(<AppearanceControls {...props} section="colors" />)
    expect(screen.getByRole('group', { name: 'Hair color' })).toBeInTheDocument()
    expect(screen.queryByRole('group', { name: 'Accessory' })).not.toBeInTheDocument()
  })
