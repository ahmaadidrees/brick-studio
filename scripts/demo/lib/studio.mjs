 ...
 */
export function hero({ name, fps, width, height, scale, track }, { zoom = 1.45, aspect = 4 / 3, out = [1200, 900], speed = Number(process.env.HERO_SPEED ?? 0.65), file = path.join(OUT, `${name}-hero`), crf = [21, 34] } = {}) {
  const base = path.join(OUT, name)
