...
import type { CharacterPalette, CharacterVisualProps } from '../types'
...
function heroPartColor(objectName: string, materialName: string, colors: CharacterPalette) {
  if (materialName === 'Grey') return objectName.startsWith('Foot')
    ? (colors.dark ?? HERO_BOOT_COLOR)
    : (colors.secondary ?? HERO_SHELL_COLOR)
  if (materialName === 'Main') return objectName.startsWith('Hand')
    ? (colors.accent ?? colors.secondary ?? HERO_GLOVE_COLOR)
    : (colors.primary ?? HERO_LIMB_COLOR)
  return colors.dark ?? HERO_VISOR_COLOR
}
...
function buildHeroVisual(source: THREE.Object3D, castShadow: boolean, colors: CharacterPalette): HeroVisual {
  const scene = cloneSkinnedScene(source)
  scene.scale.setScalar(HERO_MODEL_SCALE)
  const materialPalette = new Map<string, THREE.MeshStandardMaterial>()
  scene.traverse((object) => {
    ...
    const color = heroPartColor(mesh.name, original.name, colors)
    let material = materialPalette.get(color)
    if (!material) {
      ...
      materialPalette.set(color, material)
    }
    mesh.material = material
  })
  return { scene, materials: [...materialPalette.values()] }
}
...
export type HeroAvatarProps = CharacterVisualProps
...
export const HeroAvatar = memo(function HeroAvatar({
  ...
  const visual = useMemo(() => buildHeroVisual(sourceScene, !compact, palette), [sourceScene, compact, palette])
