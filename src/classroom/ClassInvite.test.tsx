...
it('uses the same current code in the teacher display, link and local QR', async () => {
  render(<ClassInvite classroom={{ id: 'class', name: 'STEM', code: 'NEWCODE', loginCode: 'OLDCODE', enrollmentOpen: true, collaborationOpen: true, showNamesOnJoin: true }} />)
...
it('explains closed enrollment while preserving the returning login link', () => {
  render(<ClassInvite classroom={{ id: 'class', name: 'STEM', loginCode: 'OLDCODE', enrollmentOpen: false, collaborationOpen: false, showNamesOnJoin: true }} />)
  fireEvent.click(screen.getByRole('button', { name: 'Invite students' }))
