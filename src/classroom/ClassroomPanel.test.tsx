...
beforeEach(() => { sessionStorage.clear(); localStorage.clear() }); afterEach(cleanup)
...
describe('enrollment (board 03)', () => {
  it('shows a signup password without submitting or changing the submitted credentials', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Class code'), { target: { value: 'JOIN123' } })
  ...
  it('explains the real username and password rules before sending anything', async () => {
    ...
    fireEvent.submit(document.getElementById('classroom-entry-form')!)
    expect(await screen.findByRole('alert')).toHaveTextContent('Passwords need 6 to 128 characters.')
    expect(authenticate).not.toHaveBeenCalled()
  })
  it.each([
...
it('selects a newly created class and shows its enrollment code', async () => {
  ...
  await screen.findAllByText('NEWJOIN')
...
describe('teacher roster (board 13)', () => {
  ...
  it('points an empty roster at the class codes', async () => {
    ...
    expect(screen.getAllByText('NEW-123')[0]).toBeInTheDocument()
  })
...
describe('class access and groups (board 14)', () => {
  it('shows separate code cards and copies each code', async () => {
    ...
    try {
      render(<ClassroomPanel {...props()} intent="class" client={signedIn(teacherAuth, fakeApi({ classes: [classroom] }))} />)
      fireEvent.click(await screen.findByRole('radio', { name: 'Class settings' }))
      expect(screen.getAllByText('NEW-123')[0]).toBeInTheDocument(); expect(screen.getByText('CLASS-456')).toBeInTheDocument()
  ...
  it('toggles enrollment and rotates the enrollment code without touching the sign-in code', async () => {
    ...
    expect(screen.getAllByText('NEW-789')[0]).toBeInTheDocument(); expect(screen.getByText('CLASS-456')).toBeInTheDocument()
...
describe('entry intents', () => {
  ...
  it('opens a guest on the teacher mode with Google first', () => {
    render(<ClassroomPanel {...props()} intent="teacher" client={new ClassroomClient('', vi.fn())} />)
    expect(screen.getByRole('heading', { name: 'Teacher sign in' })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: 'Continue with Google' })).toBeInTheDocument()
  ...
  it.each(['join', 'signin', 'teacher', 'worlds'] as const)('shows My Worlds to a signed-in user arriving with the %s intent', async intent => {
    ...
    expect(await screen.findByRole('radio', { name: intent === 'join' || intent === 'signin' ? 'My Class' : 'My Worlds' })).toHaveAttribute('aria-checked', 'true')
    expect(screen.queryByRole('radio', { name: 'Sign in' })).not.toBeInTheDocument()
  ...
  it.each(['save', 'worlds', 'class', 'join', 'signin', 'teacher'] as const)('sends the %s intent through the Google teacher round-trip', async intent => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Continue with Google' }))
    await waitFor(() => expect(start).toHaveBeenCalledWith(`/build?classroom=${intent === 'save' || intent === 'worlds' ? intent : 'teacher'}`))
    expect(callbacks.getDocument).toHaveBeenCalledOnce()
  })
})
...
describe('streamlined student entry', () => {
  it('preserves student fields between sign-in and account creation without storing credentials', () => {
    ...
    expect(screen.getByLabelText('Choose a username')).toHaveValue('Rover')
    expect(screen.getByLabelText('Choose a password')).toHaveValue('orbit7')
    fireEvent.change(screen.getByLabelText('Name your teacher knows'), { target: { value: 'Sam' } })
    fireEvent.click(screen.getByRole('radio', { name: 'Sign in' }))
    expect(screen.getByLabelText('Password')).toHaveValue('orbit7')
  ...
  it('allows a six-character new password and rejects obvious or matching passwords', async () => {
    ...
    render(<ClassroomPanel {...props()} intent="join" client={client} />)
    fireEvent.change(screen.getByLabelText('Class code'), { target: { value: 'ROOM42' } })
    fireEvent.change(screen.getByLabelText('Choose a username'), { target: { value: 'rover7' } })
    fireEvent.change(screen.getByLabelText('Name your teacher knows'), { target: { value: 'Sam' } })
    for (const password of ['123456', 'rover7']) {
      fireEvent.change(screen.getByLabelText('Choose a password'), { target: { value: password } })
      fireEvent.submit(document.getElementById('classroom-entry-form')!)
      expect(await screen.findByRole('alert')).toHaveTextContent('less easy-to-guess')
    }
    expect(authenticate).not.toHaveBeenCalled()
    fireEvent.change(screen.getByLabelText('Choose a password'), { target: { value: 'orbit7' } })
    fireEvent.submit(document.getElementById('classroom-entry-form')!)
    await waitFor(() => expect(authenticate).toHaveBeenCalledWith('register', expect.objectContaining({ password: 'orbit7' })))
  })
})

describe('teachers land on the invite', () => {
  it.each(['class', 'teacher'] as const)('opens a signed-in teacher arriving with the %s intent on Students with the invite expanded', async intent => {
    render(<ClassroomPanel {...props()} intent={intent} client={signedIn(teacherAuth, fakeApi({ classes: [classroom], students: [student] }))} />)
    expect(await screen.findByRole('radio', { name: 'My Class' })).toHaveAttribute('aria-checked', 'true')
    expect(screen.getByRole('radio', { name: 'Students' })).toHaveAttribute('aria-checked', 'true')
    expect(screen.getByRole('button', { name: 'Invite students' })).toHaveAttribute('aria-expanded', 'true')
    expect((screen.getByLabelText('Class link') as HTMLInputElement).value).toContain('classCode=NEW-123')
    expect(screen.getByRole('button', { name: 'Show on projector' })).toBeInTheDocument()
  })
  it('keeps the invite collapsed when a signed-in teacher comes for their worlds', async () => {
    render(<ClassroomPanel {...props()} intent="worlds" client={signedIn(teacherAuth, fakeApi({ classes: [classroom] }))} />)
    expect(await screen.findByRole('radio', { name: 'My Worlds' })).toHaveAttribute('aria-checked', 'true')
    fireEvent.click(screen.getByRole('radio', { name: 'My Class' }))
    expect(screen.getByRole('button', { name: 'Invite students' })).toHaveAttribute('aria-expanded', 'false')
    expect(screen.queryByLabelText('Class link')).not.toBeInTheDocument()
  })
  it('expands the invite after a teacher signs in with email and password', async () => {
    const client = new ClassroomClient('', fakeApi({ classes: [classroom], students: [student] }))
    vi.spyOn(client, 'authenticate').mockImplementation(async () => { client.setSession(teacherAuth); return teacherAuth })
    render(<ClassroomPanel {...props()} intent="teacher" client={client} />)
    fireEvent.click(screen.getByRole('button', { name: 'Use email and password' }))
    fireEvent.change(screen.getByLabelText('Email'), { target: { value: 'teacher@example.test' } })
    fireEvent.change(screen.getByLabelText('Password'), { target: { value: 'teacher-password' } })
    fireEvent.click(screen.getByRole('button', { name: 'Sign in' }))
    expect(await screen.findByRole('button', { name: 'Invite students' })).toHaveAttribute('aria-expanded', 'true')
    expect(screen.getByRole('radio', { name: 'Students' })).toHaveAttribute('aria-checked', 'true')
    expect(screen.getByLabelText('Class link')).toBeInTheDocument()
  })
  it('does not change where a student lands', async () => {
    render(<ClassroomPanel {...props()} intent="class" client={signedIn(auth, fakeApi({ classes: [classroom] }))} />)
    expect(await screen.findByRole('radio', { name: 'My Class' })).toHaveAttribute('aria-checked', 'true')
