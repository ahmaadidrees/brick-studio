-- Defense in depth for service-only RPC callers; application inputs are already checked.
...
create or replace function public.brick_authorize_world_batch(p_world_id uuid,p_identities jsonb)
returns jsonb language plpgsql security definer set search_path=public as $$
declare item jsonb; results jsonb:='[]'::jsonb;
begin
 if p_identities is null or jsonb_typeof(p_identities)<>'array' or jsonb_array_length(p_identities)>64 then raise exception 'Invalid permission batch'; end if;
