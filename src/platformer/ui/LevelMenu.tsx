...
export function LevelMenu(p: Props) {
  return (
    <Menu
      ...
      align="end"
      trigger={({ ref, ...props }) => (
        <Button ref={ref} variant="quiet" iconOnly icon={<MoreHorizontal size={20} />} aria-label="This level" title="This level" className="shell-world-menu-trigger" {...props}>
          This level
        </Button>
      )}
    >
      {p.onRename && <MenuItem icon={<Pencil size={18} />} label="Rename" description="Change this level’s name" onSelect={p.onRename} />}
      ...
      {p.onShareLink && <MenuItem icon={<Share2 size={18} />} label="Copy a link to this level" description="Anyone who opens it gets their own copy" onSelect={p.onShareLink} />}
      ...
      {p.onNewLevel && <MenuItem icon={<FilePlus2 size={18} />} label="New level" description="Start with a floor and a flag" onSelect={p.onNewLevel} />}
      <MenuSeparator />
