...
it('changes local preferences without touching the world or camera pose', () => {
  ...
  fireEvent.click(screen.getByRole('radio', { name: 'Free look' }))
  ...
  expect(screen.getByRole('radio', { name: 'Free look' })).toBeChecked()
...
it('offers three motion preferences, applies reduced motion and remembers it', () => {
  render(<ExploreCameraSettings />)
  fireEvent.click(screen.getByRole('button', { name: 'Settings' }))
  const group = screen.getByRole('radiogroup', { name: 'Animation preference' })
  ...
  expect(localStorage.getItem('brick-studio-motion-preference-v1')).toBe('full')
})
it('lists only shortcuts the editor really handles', () => {
  render(<ExploreCameraSettings />)
  fireEvent.click(screen.getByRole('button', { name: 'Settings' }))
  const dialog = screen.getByRole('dialog', { name: 'Settings' })
  for (const text of ['Right-drag or Space + drag', 'Shift + right-drag', 'R · F · Home', '⌘C · ⌘V · ⌘D', '1 · 2', 'Hold Shift']) {
